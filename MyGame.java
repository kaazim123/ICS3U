 ...
 *Getting Home Game
 *March 7, 2017 
 *@author Yonis Nur and Kaazim Patel
 *ICS3UP
 */
		...
		do {
			...
			if (letters[0].equalsIgnoreCase(answer)) {
                ...
                // if user chooses to go to McDonalds
					...
					c.println("As you take a seat you a guy in a mask BREAKS into the Mcdonalds with a gun. He tells you to stay put, then turns and walks away. As he walks away do you I(Be a hero and tackle him once he turns around) OR J(Do nothing and wait for him to come back");
					answer = c.readLine();
					...
					if (letters[8].equalsIgnoreCase(answer)) {
						...
						if (random == 1) {
							//outputs image
							heroImage = ImageIO.read(new File("download (3).jpg"));
			    ...
			    //if you walk
                    ...
                    if(letters[4].equalsIgnoreCase(answer)){
						c.clear();
						//output image
						robbedImage = ImageIO.read(new File ("Robbed-at-Gunpoint.jpg"));
			    	  ...
			    	  c.drawImage(fireImage, 0, 0, c.getWidth(), c.getHeight(), null);
			    	   ...
			    	   if(letters[12].equalsIgnoreCase(answer)){
			    		   ...
			    		   //if you go into living room
			    			...
			    			//so if user types the wrong answer it will output the options again
			    			if(t<=1)
			    			{
			    				c.println(">>P(key)");	
				    			c.println(">>Q(bat)");	
